...
def ASLFeat(img_name, desc_folder, N_kpts):
    
    # Import keypoints and descriptors from files .feat and .desc
    np_dsc_path = r"{}".format(img_name)
    abs_np_dsc_path = r"{}/{}".format(desc_folder, np_dsc_path)
    desc_path = abs_np_dsc_path.replace('.jpg','')
    kp = np.loadtxt('{}.feat'.format(desc_path), dtype=float, delimiter=",")
    descriptors = np.loadtxt('{}.desc'.format(desc_path), dtype=float, delimiter=",")
    descriptors = np.asarray(descriptors, np.float32)
    kp_numb = kp.shape[0]
    
    return kp, descriptors, kp_numb

def R2D2(img_name, desc_folder, N_kpts):
    np_dsc_path = Path("{}.r2d2".format(img_name))
...
def photomatch(img_name, desc_folder, N_kpts):
    if img_name[-3:] == 'jpg' or img_name[-3:] == 'JPG':
        img_name = img_name[0:-3] + 'xml'
    
    absolute_desc_path = r"{}/{}".format(desc_folder, img_name)
    fs = cv2.FileStorage(absolute_desc_path, cv2.FILE_STORAGE_READ)
...
def ImagePairs(matching_approach, image_list):
    ...
    #if matching_approach == "sequential_1img_overlap": SISTEMARE GUARDANDO IL CASO DI "sequential_2img_overlap"
    #    # Sequential matching
    #    for img1_index in range(len(image_list)-1):
    #        if (img1_index-1) >= 0:
    #            image_pairs.append((image_list[img1_index], image_list[img1_index+1]))
    #            image_pairs.append((image_list[img1_index], image_list[img1_index-1]))
    #        elif (img1_index-1) == -1:
    #            image_pairs.append((image_list[img1_index], image_list[img1_index+1]))
    #            image_pairs.append((image_list[img1_index], image_list[len(image_list)-1]))
    #    print(image_pairs)

    if matching_approach == "sequential_2img_overlap":
        # Sequential matching
        for img1_index in range(len(image_list)-2):
            image_pairs.append((image_list[img1_index], image_list[img1_index+1]))
            image_pairs.append((image_list[img1_index], image_list[img1_index+2]))
        image_pairs.append((image_list[len(image_list)-2], image_list[len(image_list)-1]))
        image_pairs.append((image_list[len(image_list)-2], image_list[0]))
        image_pairs.append((image_list[len(image_list)-1], image_list[0]))
        image_pairs.append((image_list[len(image_list)-1], image_list[1]))
        print(image_pairs)
...
def Matcher(pair_range): 
    ...
    for pair in pair_range:
        ...
        if tiling == True:
            ...
            elif local_feature == "R2D2":
                kpts_1, desc_1, kpts_numb_1 = R2D2(img1_name, desc_folder, max_kpts_per_tile)
                kpts_2, desc_2, kpts_numb_2 = R2D2(img2_name, desc_folder, max_kpts_per_tile)

            elif local_feature == "ASLFeat":
                kpts_1, desc_1, kpts_numb_1 = ASLFeat(img1_name, desc_folder, max_kpts_per_tile)
                kpts_2, desc_2, kpts_numb_2 = ASLFeat(img2_name, desc_folder, max_kpts_per_tile)

            elif local_feature == "photomatch":
                kpts_1, desc_1, kpts_numb_1 = photomatch(img1_name, desc_folder, max_kpts_per_tile)
                kpts_2, desc_2, kpts_numb_2 = photomatch(img2_name, desc_folder, max_kpts_per_tile)
