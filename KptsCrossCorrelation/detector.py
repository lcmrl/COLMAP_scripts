...
LOCAL_SUPPRESION_THRESHOLD = 15 # 15 pixels defaults
#DENSITY_THRESHOLD = 50 # 15 pixels defaults # Lower means more keypoints
DENSITY_LOWER_BOUND = 5 # 13 default # used for "density" option
DENSITY_UPPER_BOUND = 35 # 25 default # used for "density" option
...
def LocalFeatures(path_to_img, mask_threshold, silent, path_to_masks, use_ini_masks, approach, n_kpts):
    ...
    if use_ini_masks == "True":
        initial_mask = Image.open(path_to_masks / "{}.mask.{}".format(img_name[:-4], IMG_FORMAT))
        ...
        if silent == "False": convolved.show()
        convolved.save("./masks/{}.mask1.png".format(img_name))
    ...
    if approach == "random_points":
        ...
        if silent == "False": gradient_no_gaussian_pil.show()
        ...
        if silent == "False": density_map_pil.show()
        # binarization
        ...
        density_map_bin = density_map_pil.point(lambda p: 255 if p > DENSITY_LOWER_BOUND and p < DENSITY_UPPER_BOUND else 0 )
        density_map_bin = density_map_bin.convert("1")
        if silent == "False": density_map_bin.show()
        #initial_mask = ImageChops.invert(initial_mask)
        convolved = Image.fromarray(convolved)
        ...
        if silent == "False": density_map_bin.show()
        density_map_bin.save("./masks/{}.mask2.png".format(img_name))
...
def main():
    ...
    kpts_dict = {}
    for img in img_list:
        print("Current image: {}".format(img))
