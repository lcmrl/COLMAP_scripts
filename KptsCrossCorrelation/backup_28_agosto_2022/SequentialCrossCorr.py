...
def SequentialCrossCorrelation(initial_kpt, target_image, img_folder, patch_width, patch_hight, silent, img_list):
    ...
    y, x, max_ncc = NCC(searching_window, template_patch, silent)
...
def Tracking(tentative_kpts, img_folder, img_dict, patch_width, patch_hight, silent, img_list): # TODO si potrebbe ciclare su patch successive aggiorando la patch reference con quella appena trovata e così via
    ...
    for kp in tentative_kpts:
        ...
        for c in candidate_images:
            ...
            x, y, max_ncc = SequentialCrossCorrelation(kp, target_im, img_folder, patch_width, patch_hight, silent, img_list)
            if np.absolute(x-kp[1])>=STATIC_KPTS_THRESHOLD and np.absolute(y-kp[2])>=STATIC_KPTS_THRESHOLD and max_ncc>NCC_REJECT_THRESHOLD:
